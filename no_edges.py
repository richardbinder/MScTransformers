...
class EncOnlyTransformerRegressor(nn.Module):
    def __init__(self, in_dim, hidden_dim=256, n_layers=4, heads=8, dropout=0.1):
        super().__init__()
    ...
    def forward(self, x, batch):
        # x is [total_nodes_in_batch, in_dim]
        h = self.in_proj(x)

        # We must pad to [B, Nmax, hidden_dim] for nn.TransformerEncoder
        B = int(batch.max().item()) + 1
        counts = torch.bincount(batch, minlength=B)
        Nmax = int(counts.max().item())

        # Build padded tensor + mask
        padded = h.new_zeros((B, Nmax, h.size(-1)))
        pad_mask = torch.ones((B, Nmax), dtype=torch.bool, device=h.device)

        start = 0
        for i, n in enumerate(counts.tolist()):
            padded[i, :n] = h[start:start+n]
            pad_mask[i, :n] = False
            start += n

        # Transformer with key padding mask
        z = self.encoder(padded, src_key_padding_mask=pad_mask)  # [B, Nmax, hidden]

        # Mean over valid nodes
        z = z.masked_fill(pad_mask.unsqueeze(-1), 0.0)
        denom = (~pad_mask).sum(dim=1).clamp(min=1).unsqueeze(-1)
        g = z.sum(dim=1) / denom  # [B, hidden]
...
def main():
    ...
    model = EncOnlyTransformerRegressor(
        in_dim=in_dim,
